...
class MyBinanceManager:
    def __init__(self, logger:IndentedLoggerAdapter=None) -> None:
        # Setup logger
        self.log = utl.get_indentedLogger() if logger is None else logger
    ...
    def getServerLocalTimeOffset(self) -> float:
        ...
        return (self.get_server_time() /
                Constants.SERVER_TO_EPOCH_TIME_DIVISOR) - utl.get_local_time()
    ...
    def get_symbol_info(self, symbolName: str) -> Symbol:
        return Symbol(
            SymbolInfo=RUC(self.client.get_symbol_info(symbol=symbolName)))

    @BIN_RETRY
    def get_klines(self, symbolName: str, interval: str, limit: int,) -> KlinesDataframe:
    ...
    @BIN_RETRY
    def get_historical_klines(self, symbolName: str, interval: str,
                              startTime: str) -> KlinesDataframe:
        return KlinesDataframe(
            RUC(
                self.client.get_historical_klines(symbol=symbolName,
                                                  interval=interval,
                                                  start_str=startTime)))
    ...
    def getAllOpenStopLossLimitOrders(self):
        stopLossOrders = list(
            filter(lambda x: x.get('type') == 'STOP_LOSS_LIMIT',
    ...
    def getMarketDepthRatio(self,
        ...
        if float(asks[-1][0]) >= asks_price or float(
                bids[-1][0]) <= bids_price:
            raise Exception(
        ...
        ask_qty = sum([
            float(ask[1]) * float(ask[0]) for ask in asks
            if float(ask[0]) <= asks_price
        ])
        bid_qty = sum([
            float(bid[1]) * float(bid[0]) for bid in bids
    ...
    def getOrderStatus(self, symbolName: str, orderId: int) -> str:
        return self.get_order(symbolName=symbolName,
                              orderId=orderId).get('status')

    def cancel_order(self, symbolName: str, orderId: int):
        return RUC(self.client.cancel_order(symbol=symbolName,
                                            orderId=orderId))

    def order_limit_sell(self,
                         symbolName: str,
                         qty: float,
                         sellPrice: float,
                         recvWindow=1000) -> dict:
        return RUC(
            self.client.order_limit_sell(symbol=symbolName,
                                         quantity=qty,
                                         price=sellPrice,
    ...
    def order_limit_buy(self,
                        ...
                        recvWindow=1000) -> dict:
        return RUC(
            self.client.order_limit_buy(symbol=symbolName,
                                        quantity=qty,
                                        price=buyPrice,
                                        recvWindow=recvWindow))

    def createStopLossSellOrder(self, symbolName: str, qty: float,
                                stopPrice: float, limitPrice: float) -> dict:
        return RUC(
            self.client.create_order(symbol=symbolName,
                                     side='SELL',
    ...
    def createOCOSellOrder(self, symbolName: str, qty: float, gainPrice: float,
                           stopPrice: float, stoplimitPrice: float) -> dict:
        return RUC(
