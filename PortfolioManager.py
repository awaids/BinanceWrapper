...
class PortfolioManager:
    ...
    def prunedPortfolio(self) -> pd.DataFrame:
        ...
        idx_to_prune = [
            idx
            for idx, row in self.completePortfolio.iterrows()
            if float(row["CurrentWorth"]) < 10.0
        ]
    ...
    def getDepositedFiatUSD() -> float:
        ...
        def get_eur_day_price(time: int) -> float:
            ...
            return _local_bm.get_historical_klines(
                symbolName="EURUSDT", interval="1d", startTime=date
            ).df.at[0, "Close"]
        ...
        for deposit in deposits:
            ...
            if deposit.get("status") != "Successful":
                continue
            total += (
                float(deposit.get("amount")) * get_eur_day_price(orderTime)
                if deposit.get("fiatCurrency") == "EUR"
                else float(deposit.get("amount"))
            )
        return total
    ...
    def getCompletePortfolio() -> pd.DataFrame:
        ...
        df.sort_values(
            by=["CurrentWorth"], ignore_index=True, ascending=False, inplace=True
        )
        return df
